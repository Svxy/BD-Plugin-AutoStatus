class AnimatedStatus {
    ...
    start() {
        if (this.animation.length == 0)
            BdApi.showToast("No status set. Go to Settings>Plugins to set a custom animation!");
        else
            this.AnimationLoop();
    }
    ...
    strerror: (req) => {
        ...
        for (const s of["errors", "custom_status", "text", "_errors", 0, "message"])
            if ((json == undefined) || ((json = json[s]) == undefined))
                return "Unknown error. Please report at github.com/svxy";
